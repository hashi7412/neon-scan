...
// Modified by: Leo Pawel 	<https://github.com/galaxy126>
// 07/09/2022
...
export class Opcode {
	...
	isMemoryAccess(readOrWrite?: boolean): OpcodeMemoryAccess|null {
...
const Opcodes: Array<Opcode|null> = [ ];
...
const hexlify = (v: number|Uint8Array) => {
	if (typeof v==='number') {
		let hex = "";
        while (v) {
            hex = HexCharacters[v & 0xf] + hex;
            v = Math.floor(v / 16);
        }
	...
	for (let i = 0; i < v.length; i++) {
		let n = v[i];
		result += HexCharacters[(n & 0xf0) >> 4] + HexCharacters[n & 0x0f];
	}
	return result;
}

export const disassemble = (bytecode: string) => {
	const ops: Array<Operation> = [ ];
	const offsets: { [ offset: number ]: Operation } = { }
	const bytes = hexStringToByteArray(bytecode)
	let i = 0;
	let oob = false;
	while (i < bytes.length) {
		let opcode = Opcode.from(bytes[i]);
		if (!opcode) {
			opcode = new Opcode(`unknown (${hexlify(bytes[i])})`, bytes[i], 0, 0);
		...
		if (push) {
			const data = hexlify(bytes.slice(i, i + push))
